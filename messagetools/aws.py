...
class AWS(object):
    ...
    # sign, possibly encrypt, send message and context
    # args: 
    @safe_sns
    def send_message(self, msg, context, cryptid, signid, arn=None):
        sns_client = boto3.client('sns')
        b64msg = encode_message(msg, context, cryptid, signid)
        if arn is None:
            arn = self._conf['SNS_ARN']
        rsp = sns_client.publish(TopicArn=arn, Message=b64msg, Subject='iam-message', MessageStructure='string')
        # plus message attrs
        return _status(rsp)
    ...
    def recv_message(self, queue):
        ...
        for rawmsg in rsp['Messages']:
            try:
               dec_msgs.append((rawmsg['ReceiptHandle'], decode_message(rawmsg['Body'])))
            except:
               # print('not an iam message: ' + body)
               pass
        return _status(rsp), dec_msgs
    ...
    # returns num_messages, num_processed
    @safe_sqs
    def recv_and_process(self, handler, queue, max=20):
        ...
        for m in msgs:
            ret = handler(msg)
            if ret:
                sqs_queue.delete_message(m['ReceiptHandle'])
                num_valid += 1
        return 200, nmsg, num_valid
